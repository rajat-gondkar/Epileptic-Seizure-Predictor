#!/usr/bin/env python3
"""
Genetic Feature Engineering Module (v2 — Extended)
====================================================
Constructs genetic profile vectors for each patient.

Feature Vector (22 dimensions):
    [0-8]   Weighted mutation flags for 9 epilepsy genes (continuous: 0 or risk weight)
    [9-15]  pLI scores for 7 genes (continuous: 0-1)
    [16]    Polygenic Risk Score (standardized)
    [17]    Sodium channel interaction (SCN1A × SCN2A × SCN8A)
    [18]    Potassium channel interaction (KCNQ2 × KCNT1)
    [19]    Receptor interaction (GABRA1 × GRIN2A)
    [20]    PRS × Tier-1 mutation interaction
    [21]    Mutation burden (sum of weighted mutations)
...
# Extended pLI genes (7 genes with high pLI scores)
EXTENDED_PLI_GENES = ['SCN1A', 'SCN8A', 'KCNQ2', 'SCN2A', 'GRIN2A', 'PCDH19', 'GABRA1']

# Gene risk tiers
TIER1_GENES = ['SCN1A', 'KCNQ2', 'SCN2A']
TIER2_GENES = ['SCN8A', 'KCNT1']
...
FEATURE_NAMES = [f'{g}_mutation' for g in TARGET_GENES] + \
                [f'{g}_pLI' for g in EXTENDED_PLI_GENES] + \
                ['polygenic_risk_score',
                 'sodium_channel_interaction',
                 'potassium_channel_interaction',
                 'receptor_interaction',
                 'prs_tier1_interaction',
                 'mutation_burden']
...
def build_mutation_flags(patient_variants=None, clinvar_df=None,
    ...
    """
    Build weighted mutation flags for the 9 target genes.
    ...
    Returns:
        np.array of shape [9], weighted flags (0 or risk weight)
    """
    if rng is None:
    ...
    if patient_variants is not None and not simulated:
        for i, gene in enumerate(TARGET_GENES):
            ...
            if len(gene_variants) > 0:
                flags[i] = GENE_RISK_WEIGHTS.get(gene, 0.5)
    else:
        for i, gene in enumerate(TARGET_GENES):
...
# ============================================================
def compute_interaction_features(binary_flags, prs):
    """
    Compute gene-gene interaction features.

    Args:
        binary_flags: dict of {gene: 0 or 1}
        prs: float, polygenic risk score

    Returns:
        dict of interaction features
    """
    # Sodium channel epistasis (SCN1A × SCN2A × SCN8A)
    sodium_interaction = float(
        binary_flags.get('SCN1A', 0) *
        binary_flags.get('SCN2A', 0) *
        binary_flags.get('SCN8A', 0)
    )

    # Potassium channel interaction (KCNQ2 × KCNT1)
    potassium_interaction = float(
        binary_flags.get('KCNQ2', 0) *
        binary_flags.get('KCNT1', 0)
    )

    # Receptor interaction (GABRA1 × GRIN2A) — excitation/inhibition balance
    receptor_interaction = float(
        binary_flags.get('GABRA1', 0) *
        binary_flags.get('GRIN2A', 0)
    )

    # PRS × Tier-1 mutation interaction
    tier1_flag = int(any(binary_flags.get(g, 0) == 1 for g in TIER1_GENES))
    prs_tier1_interaction = float(prs * tier1_flag)

    # Mutation burden (sum of weighted mutations)
    mutation_burden = sum(
        GENE_RISK_WEIGHTS.get(g, 0.5) * binary_flags.get(g, 0)
...
def build_genetic_vector(patient_id, clinvar_df=None, gnomad_df=None,
    ...
    """
    Build the complete 22-dimensional genetic feature vector for a patient.
    ...
    Returns:
        np.array of shape [22]
    """
    if seed is None:
        seed = hash(patient_id) % (2**31)
    rng = np.random.RandomState(seed)

    # [0-8] Weighted mutation flags (9 features)
    ...
    # Convert to binary dict for interaction computation
    binary_flags = {}
    for i, gene in enumerate(TARGET_GENES):
        binary_flags[gene] = 1 if mutation_flags[i] > 0 else 0

    # [9-15] pLI scores (7 features)
    pli_scores = get_pli_scores(gnomad_df, genes=EXTENDED_PLI_GENES)

    # [16] Polygenic Risk Score (1 feature)
    prs = compute_prs(
        patient_snp_dosages=None,
        snp_weights_df=gwas_df,
        rng=rng
    )

    # [17-21] Interaction features (5 features)
    interactions = compute_interaction_features(binary_flags, prs)

    # Concatenate all features
    genetic_vector = np.concatenate([
        mutation_flags,           # [0-8]
        pli_scores,              # [9-15]
        [prs],                   # [16]
        [interactions['sodium_channel_interaction']],    # [17]
        [interactions['potassium_channel_interaction']], # [18]
        [interactions['receptor_interaction']],          # [19]
        [interactions['prs_tier1_interaction']],         # [20]
        [interactions['mutation_burden']],               # [21]
    ])
...
def generate_genetic_profiles(patients, clinvar_df=None, gnomad_df=None,
    ...
    for patient_id in patients:
        ...
        for i, gene in enumerate(TARGET_GENES):
            record[f'{gene}_mutation'] = float(vector[i])
        for i, gene in enumerate(EXTENDED_PLI_GENES):
            record[f'{gene}_pLI'] = float(vector[9 + i])
        record['polygenic_risk_score'] = float(vector[16])
        record['sodium_channel_interaction'] = float(vector[17])
        record['potassium_channel_interaction'] = float(vector[18])
        record['receptor_interaction'] = float(vector[19])
        record['prs_tier1_interaction'] = float(vector[20])
        record['mutation_burden'] = float(vector[21])
...
def main():
    ...
    print("=" * 60)
    print("GENETIC FEATURE ENGINEERING (v2 — Extended)")
    print("=" * 60)
